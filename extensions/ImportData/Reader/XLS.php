...
class ImportData_Reader_XLS {
    ...
    var $_defaultFormat = self::SPREADSHEET_EXCEL_READER_DEF_NUM_FORMAT;
     ...
     */
    function ImportData_Reader_XLS($file='',$store_extended_info=true,$outputEncoding='') {
        $this->_ole = new ImportData_Reader_OLERead();
    ...
    function _parse() {
        ...
        $code = Commons::v($data,$pos);
        $length = Commons::v($data,$pos+2);
        $version = Commons::v($data,$pos+4);
        $substreamType = Commons::v($data,$pos+6);
            ...
            switch ($code) {
                case self::SPREADSHEET_EXCEL_READER_TYPE_SST:
                    ...
                    for ($i = 0; $i < $uniqueStrings; $i++) {
                        ...
                        if ($spos == $limitpos) {
                            $opcode = Commons::v($data,$spos);
                            $conlength = Commons::v($data,$spos+2);
                            if ($opcode != 0x3c) {
                                return -1;
                            }
                            $spos += 4;
                        ...
                        if ($richString) {
                            // Read in the crun
                            $formattingRuns = Commons::v($data,$spos);
                            $spos += 2;
                        }
                ...
                case self::SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                    $indexCode = Commons::v($data,$pos+4);
                    if ($version == self::SPREADSHEET_EXCEL_READER_BIFF8) {
                        $numchars = Commons::v($data,$pos+6);
                        if (ord($data[$pos+8]) == 0){
                ...
                case self::SPREADSHEET_EXCEL_READER_TYPE_FONT:
                    $height = Commons::v($data,$pos+4);
                    $option = Commons::v($data,$pos+6);
                    $color = Commons::v($data,$pos+8);
                    $weight = Commons::v($data,$pos+10);
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
    ...
    function _getCellDetails($spos,$numValue,$column) {
        ...
        if ($type == 'date') {
            ...
            $dateinfo = Commons::gmgetdate($utcValue);
